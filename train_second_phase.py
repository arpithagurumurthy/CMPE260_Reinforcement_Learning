...
def hold(actions):
    ...
    if next_probable_action == 2 and len(agent.inventory) > 0:
        ...
        if max_profit > 0:
            ...
            return 'Hold', actions
    # elif next_probable_action == 1 and agent.balance > stock_prices[t] and len(agent.inventory) > 0:
    #     best_buy = stock_prices[t] - min(agent.inventory)
    #     if best_buy < 0:
    #         buy(t)
    #         actions[next_probable_action] = 1 # reset this action's value to the highest
    #         return 'Hold', actions
...
# configure logging
logging.basicConfig(filename=f'logs/{model_name}_training_{stock_name}_second_phase.log', filemode='w',
...
for e in range(1, num_episode + 1):
    ...
    if e % 2 == 0:
        agent.model.save('saved_models/DQN_second_draft_ep' + str(e) + '.h5')
