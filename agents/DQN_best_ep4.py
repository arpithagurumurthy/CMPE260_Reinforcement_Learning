...
class Agent(Portfolio):
    def __init__(self, state_dim, balance, is_eval=False, model_name=""):
        ...
        ##updated maxlen from 1000 to 10000
        self.memory = deque(maxlen=10000)
        ##updated buffer size from 60 to 30
        self.buffer_size = 30
    ...
    def model(self):
        ...
        # model.compile(loss=Huber(delta=1.5), optimizer=Adam(lr=0.01))
        # return model
    ...
    def experience_replay(self):
        ...
        ##Updated to randomly sample the experiences
        mini_batch = random.sample(self.memory, self.buffer_size)
        for state, actions, reward, next_state, done in mini_batch:
            if not done:
                ##Updated the bellman equation
